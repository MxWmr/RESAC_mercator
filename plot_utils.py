...
def plot_test_ssh(l_im,save_path,date,cmap="coolwarm",save=True):
    ...
    for n,line in enumerate(l_im):
        [ssh3,ssh6,ssh12,_,_,target,_,_] = line
...
def plot_test_uv(l_im,save_path,date,save=True):
    ...
    for n,line in enumerate(l_im):
        ...
        im = axes[n,0].quiver(torch.squeeze(u_pred).cpu().numpy(),torch.squeeze(v_pred).cpu().numpy(),color='g')
        im = axes[n,1].quiver(torch.squeeze(u).cpu().numpy(),torch.squeeze(v).cpu().numpy(),color='g')

    
    axes[0,0].set_title('pred currents')
    axes[0,1].set_title('true currents')
